...
class Util
{
  ...
  public const SECRETARIO_DESPACHO = "Secretario_Despacho";
  public const ALCALDE = "Alcalde";
  ...
  public static function getColorUbicacionActual()
  {
    return "#62af0a";
  }
  ...
  public static function getSecretariaPrincipal()
  {
    return "2";
  }
  ...
  {
      $query = "SELECT municipio FROM tbl_ciudades_accion_unificada WHERE codigo_muncipio = '{$codigoMunicipio}' LIMIT 1";
      $row = self::sb_db_get($query, true);

      if (isset($row['municipio']) && !empty($row['municipio'])) {
          return ucfirst(mb_strtolower(trim($row['municipio'])));
      } else {
  ...
  public static function getCodigoVeredadMunicipioPrincipal()
  {
    return "9581"; //9581 86011
  }
  public static function getIdentificadorPilarPrincipal()
  {
    return "1";
  }
    public static function getSecretariaIdTIC()
  {
    return 10;
  }
  ...
  public static function getIdentificadorDepartamentoPrincipal()
  {
    return "27";
  }
  public static function codigoTodos()
  {
    return 10000;
  }
  public static function Secretario_Despacho()
  {
    return "Secretario_Despacho";
  }
  public static function Alcalde()
  {
    return "Alcalde";
  }
  public static function Auxiliar()
  {
    return "Auxiliar";
  }
  public static function Auxiliar_Alcalde()
  {
    return "Auxiliar_Alcalde";
  }
  ...
  /**
   * Método para obtener MARGE DE ERROR FICHA TECNICA ENCUESTA
   */
  public static function calcularMargenError($poblacion, $muestra, $valorZ) {
    $margen = $valorZ * sqrt(0.25 / $muestra) * sqrt(($poblacion - $muestra) / ($poblacion - 1));
    return ceil($margen * 100 * 100) / 100; // Redondea hacia arriba con 2 decimales
  }

  public static function sb_db_get($query, $single = true)
    {
        $db     = new DbConection();
        $pdo    = $db->openConect();
        $value  = ($single ? '' : []);
  ...
  public static function getInformacionConfiguracion()
  {
      $db = new DbConection();
      $q = "SELECT * FROM " . $db->getTable('tbl_configuracion') . " ORDER BY id DESC LIMIT 1";
      return Util::sb_db_get($q, false);
  }
   ...
   *
   */
  public static function make_query_update_old($table, $where, $arrfieldscomma, $arrfieldsnocomma)
  {
    $query = "UPDATE ";
  ...
  public static function make_query_update($table, $where, $arrfieldscomma, $arrfieldsnocomma)
  {
    ...
    // Validaciones de entrada más robustas usando empty()
    if (empty($table)) {
      return "***Falta nombre de la tabla***";
    }
    if (empty($where)) {
    ...
    // Construcción de los campos que requieren comillas (para cadenas)
    foreach ($arrfieldscomma as $f => $v) {
      // Usar !is_null($v) para asegurarnos de que no es NULL antes de usar empty() o para asignar NULL directamente
      // Si el objetivo es NO actualizar el campo si el valor es NULL o vacío, entonces sigue como estaba
      // Si el objetivo es establecer el campo a NULL en la DB si el PHP recibe NULL o una cadena vacía, entonces:
      // $fields .= " " . $f . " = " . (is_null($v) || $v === '' ? 'NULL' : "'" . $v . "'") . ",";

      // Para mantener el comportamiento original de solo agregar si tiene valor (no null y no vacío)
      if (!empty($v)) { // Esto manejará null, '', 0, false, etc. como vacío.
          $fields .= " " . $f . " = '" . $v . "',";
      }
      // Considera si quieres que un campo vacío en PHP establezca el campo de la DB a NULL.
      // Si es así, la lógica sería diferente aquí.
    }

    // Construcción de los campos que no requieren comillas (para números o funciones SQL)
    foreach ($arrfieldsnocomma as $f2 => $v2) {
      // Aquí se asume que $v2 es numérico o una función SQL.
      // Si $v2 pudiera ser 0 o un string vacío que quieres ignorar, usa !empty($v2)
      if ($v2 > 0) { // Esta condición significa que solo se agregará si es un número positivo.
        $fields .= " " . $f2 . " = " . $v2 . ",";
      }
    }

    $fields = rtrim($fields, ","); // Elimina la última coma
    $query .= $fields . " WHERE " . $where;
  ...
  public static function error_missing_data_description($description = '')
  {
    return array('output' => array('valid' => false, 'response' => array('code' => '10033', 'content' => ' ❌ Faltan datos que son requeridos. ' . $description)));
  }
  ...
  public static function trace_session_user($rqst)
  {
      $db = new DbConection();
      $pdo = $db->openConect();
      ...
      $user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
      $usuario_id = isset($rqst['usuarioId']) ? intval($rqst['usuarioId']) : 0;
      $ip_address = Util::get_real_ipaddress();

      // Preparar la consulta
      $q = "INSERT INTO " . $db->getTable('tbl_historial_session') . " 
            (dtcreate, ip, navegador, tec_usuario_id) 
            VALUES (:dtcreate, :ip, :navegador, :tec_usuario_id)";
      ...
      try {
          ...
          $stmt->execute([
              ...
              ':tec_usuario_id' => $usuario_id,
          ]);
      } catch (PDOException $e) {
          error_log("Error al insertar en tbl_historial_session: " . $e->getMessage());
      } finally {
          $db->closeConect();
      }
  }
  ...
  public static function idDepartamentoObjetivo()
  {
    return 27;
  }
  ...
  /**
   * Generar URL para el mapa general de ciudades por código de departamento y código de municipio
   * @param string $phpRuta La ruta base del archivo PHP que maneja el mapa
   * @param string $codigoDepartamento El código del departamento
   * @param string $codigoMunicipio El código del municipio
   * @param string $nombreMunicipio (Opcional) El nombre del municipio
   * @return string La URL generada
   */
  public static function generarUrlMapaGeneralCiudadesPorCodDepartamentoYCodMunicipio($phpRuta, $codigoDepartamento, $codigoMunicipio, $nombreMunicipio = '') {
      // Construir la URL con los parámetros
      $url = $phpRuta . '?mun=' . urlencode($codigoMunicipio) .
            '&dep=' . urlencode($codigoDepartamento);
      
      // Agregar nombre si está disponible
      if (!empty($nombreMunicipio)) {
          $url .= '&nombre=' . urlencode($nombreMunicipio);
      }
