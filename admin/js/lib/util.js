var UTIL = {
    scrollToTop: function(formulario, time = 500) {
        $("html, body").animate(
    ...
    /**
     * Metodo para obtener los parametros de la URL para Departamento, Municipio y Vereda
     * @returns {JSON} {mun, dep}
     */
    getParamsFromUrlDepartamentoMunicipio: function() {
        const urlParams = new URLSearchParams(window.location.search);
        const mun = urlParams.get('mun');
        const dep = urlParams.get('dep');
        const vereda = urlParams.get('id');
        const pilar = urlParams.get('pilar');
        return { mun, dep, vereda, pilar };
    },
...
function calcularMargenError(poblacion, muestra, valorZ) {
  // Validar que los parámetros sean números válidos
  if (isNaN(poblacion) || isNaN(muestra) || isNaN(valorZ)) {
    return 0;
  }

  // Validar que sean valores positivos
  if (poblacion <= 0 || muestra <= 0 || valorZ <= 0) {
    return 0;
  }

  // Validar que la muestra no sea mayor que la población
  ...
  const margen =
    valorZ *
    Math.sqrt(0.25 / muestra) *
    Math.sqrt((poblacion - muestra) / (poblacion - 1));
  ...
  return Math.ceil(margen * 100 * 100) / 100;
}